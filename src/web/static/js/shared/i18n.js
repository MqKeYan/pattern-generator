// 国际化：五语言词典与翻译逻辑（前后台共用）

// zh-CN 词典
window.I18N_LOCALES = window.I18N_LOCALES || {};
window.I18N_LOCALES['zh-CN'] = {
    app_title: '斑图形成可视化系统',
    loading: '加载中...',
    model_select: '模型选择',
    params_setting: '参数设置',
    reset_params: '重置参数',
    init_range: '初始值范围',
    x_min: 'X 最小值',
    x_max: 'X 最大值',
    y_min: 'Y 最小值',
    y_max: 'Y 最大值',
    apply_best_init: '应用推荐值',
    track_points: '时间演化跟踪点',
    coord_x: 'X坐标',
    coord_y: 'Y坐标',
    add: '添加',
    clear: '清空',
    edit: '编辑',
    delete: '删除',
    iterations: '迭代次数',
    reset_iters: '应用推荐值',
    anim_setting: '动画设置',
    anim_start: '起始迭代',
    anim_end: '结束迭代',
    anim_frames: '帧数',
    control_panel: '控制面板',
    run_sim: '运行模拟',
    run_anim: '动画模拟',
    reset_all: '重置所有',
    clean_cache: '清理缓存',
    software_settings: '软件设置',
    language: '语言',
    hardware_label: '计算硬件',
    client_label: '客户端',
    client_name: '客户端名称',
    language_zh_cn: '简体中文',
    language_zh_tw: '繁体中文',
    language_en: '英文',
    language_ja: '日语',
    language_ko: '韩语',
    restore_default: '恢复默认',
    save: '保存',
    cancel: '取消',
    status_ready: '就绪，请选择参数运行模拟',
    tab_2d: '二维斑图',
    tab_3d: '三维斑图',
    tab_anim: '动画演示',
    play: '播放',
    pause: '暂停',
    speed: '速度',
    computing: '计算中...',
    point_range_error: '坐标必须在0~99之间',
    iter_max_warning: '迭代次数不能超过{max}，已自动调整为{max}',
    point_exists: '点({x},{y})已存在',
    simulating_status: '模拟进行中...',
    sim_complete: '模拟完成 — {model}，迭代{iters}次',
    sim_done: '模拟完成！',
    sim_failed: '模拟失败: {msg}',
    init_failed: '初始化失败',
    restored_sim: '已恢复上次模拟结果',
    restored_toast: '已恢复上次结果，刷新无忧',
    reset_done: '所有设置已重置',
    status_reset_all: '本客户端已重置所有设置',
    status_cache_cleared: '本客户端缓存已清理',
    status_cache_clear_failed: '本客户端缓存清理失败',
    status_task_cancel_requested: '已请求取消本客户端任务',
    status_settings_saved: '本客户端软件设置已保存',
    client_task_log_title: '本客户端任务日志',
    client_task_log_desc: '仅显示本窗口客户端提交的任务日志。',
    client_task_log_close: '关闭',
    client_task_log_loading: '正在加载本客户端任务日志...',
    client_task_log_empty: '当前客户端暂无可显示的任务日志。',
    clean_failed: '清理失败: {msg}',
    need_anim: '请先运行动画计算',
    anim_ready: '动画数据准备完成',
    anim_ready_status: '动画数据就绪，点击播放',
    anim_failed_status: '动画准备失败',
    anim_preparing: '准备动画数据...',
    track_list_center: '当前跟踪点: 中心点(50,50)',
    frame_count: '帧: {current} / {total}',
    iterations_axis: '迭代次数',
    density_axis: '种群密度',
    axis_x: 'X轴',
    axis_y: 'Y轴',
    center_evo_title: '中心点时间演化',
    center_x: 'X种群-中心点',
    center_y: 'Y种群-中心点',
    density: '密度',
    anim_title_x: 'X种群 - 迭代 {iter}',
    anim_title_y: 'Y种群 - 迭代 {iter}',
    anim_title_combined: '合并斑图 - 迭代 {iter}',
    model_1: '模型1·R-M型',
    model_2: '模型2·Holling II型',
    model_3: '模型3·比值依赖型',
    model_4: '模型4·对称竞争',
    model_5: '模型5·连续化离散型',
    task_queued: '已加入队列，前面还有 {n} 个任务',
    status_queued: '排队中',
    status_running: '运行中',
    status_completed: '已完成',
    status_cancelled: '已取消',
    status_timeout: '已超时',
    status_failed: '已失败',
    status_expired: '已过期',
    type_simulate: '模拟',
    type_animate: '动画',
    info_version: '软件版本',
    info_client_id: '客户端号',
    info_python: 'Python 版本',
    info_cuda: 'CUDA 版本',
    info_pytorch: 'Pytorch 版本',
    info_cpu: 'CPU 型号',
    info_gpu: 'GPU 型号',
    info_hardware: '计算硬件',
    client_online: '在线',
    client_offline: '离线',
    client_paused: '已暂停',
    client_queued: '排队中',
    client_computing: '计算中',
    task_running: '执行中，进度 {p}%',
    cancel_task: '取消任务',
    task_cancelled: '任务已取消',
    task_cancel_failed: '取消失败: {msg}',
    task_paused: '已加入队列，当前被管理员暂停',
    task_failed_status: '任务失败: {msg}',
    task_timeout_status: '任务超时，请重试',
    task_expired_status: '任务结果已过期，请重新运行',
    task_not_found: '任务不存在',
};

// zh-TW 词典
window.I18N_LOCALES = window.I18N_LOCALES || {};
window.I18N_LOCALES['zh-TW'] = {
    app_title: '斑圖形成可視化系統',
    loading: '載入中...',
    model_select: '模型選擇',
    params_setting: '參數設置',
    reset_params: '重置參數',
    init_range: '初始值範圍',
    x_min: 'X 最小值',
    x_max: 'X 最大值',
    y_min: 'Y 最小值',
    y_max: 'Y 最大值',
    apply_best_init: '套用推荐值',
    track_points: '時間演化追蹤點',
    coord_x: 'X座標',
    coord_y: 'Y座標',
    add: '新增',
    clear: '清空',
    edit: '編輯',
    delete: '刪除',
    iterations: '疊代次數',
    reset_iters: '應用建議值',
    anim_setting: '動畫設置',
    anim_start: '起始疊代',
    anim_end: '結束疊代',
    anim_frames: '幀數',
    control_panel: '控制面板',
    run_sim: '執行模擬',
    run_anim: '動畫模擬',
    reset_all: '全部重置',
    clean_cache: '清理快取',
    software_settings: '軟件設置',
    language: '語言',
    hardware_label: '計算硬件',
    client_label: '客戶端',
    client_name: '客戶端名稱',
    language_zh_cn: '簡體中文',
    language_zh_tw: '繁體中文',
    language_en: '英文',
    language_ja: '日語',
    language_ko: '韓語',
    restore_default: '恢復默認',
    save: '保存',
    cancel: '取消',
    status_ready: '就緒，請選擇參數執行模擬',
    tab_2d: '二維斑圖',
    tab_3d: '三維斑圖',
    tab_anim: '動畫演示',
    play: '播放',
    pause: '暫停',
    speed: '速度',
    computing: '計算中...',
    point_range_error: '座標必須在0~99之間',
    iter_max_warning: '迭代次數不能超過{max}，已自動調整為{max}',
    point_exists: '點({x},{y})已存在',
    simulating_status: '模擬進行中...',
    sim_complete: '模擬完成 — {model}，疊代{iters}次',
    sim_done: '模擬完成！',
    sim_failed: '模擬失敗: {msg}',
    init_failed: '初始化失敗',
    restored_sim: '已恢復上次模擬結果',
    restored_toast: '已恢復上次結果，重新整理無憂',
    reset_done: '所有設置已重置',
    status_reset_all: '本用戶端已重置所有設置',
    status_cache_cleared: '本用戶端快取已清理',
    status_cache_clear_failed: '本用戶端快取清理失敗',
    status_task_cancel_requested: '已請求取消本用戶端任務',
    status_settings_saved: '本用戶端軟件設置已儲存',
    client_task_log_title: '本用戶端任務日誌',
    client_task_log_desc: '僅顯示本視窗用戶端提交的任務日誌。',
    client_task_log_close: '關閉',
    client_task_log_loading: '正在載入本用戶端任務日誌...',
    client_task_log_empty: '目前用戶端暫無可顯示的任務日誌。',
    clean_failed: '清理失敗: {msg}',
    need_anim: '請先執行動畫計算',
    anim_ready: '動畫資料準備完成',
    anim_ready_status: '動畫資料就緒，點擊播放',
    anim_failed_status: '動畫準備失敗',
    anim_preparing: '準備動畫資料...',
    track_list_center: '目前追蹤點: 中心點(50,50)',
    frame_count: '幀: {current} / {total}',
    iterations_axis: '疊代次數',
    density_axis: '種群密度',
    axis_x: 'X軸',
    axis_y: 'Y軸',
    center_evo_title: '中心點時間演化',
    center_x: 'X種群-中心點',
    center_y: 'Y種群-中心點',
    density: '密度',
    anim_title_x: 'X種群 - 疊代 {iter}',
    anim_title_y: 'Y種群 - 疊代 {iter}',
    anim_title_combined: '合併斑圖 - 疊代 {iter}',
    model_1: '模型1·R-M型',
    model_2: '模型2·Holling II型',
    model_3: '模型3·比值依賴型',
    model_4: '模型4·對稱競爭',
    model_5: '模型5·連續化離散型',
    task_queued: '已加入佇列，前面還有 {n} 個任務',
    status_queued: '排隊中',
    status_running: '執行中',
    status_completed: '已完成',
    status_cancelled: '已取消',
    status_timeout: '已逾時',
    status_failed: '已失敗',
    status_expired: '已過期',
    type_simulate: '模擬',
    type_animate: '動畫',
    info_version: '軟體版本',
    info_client_id: '用戶端號',
    info_python: 'Python 版本',
    info_cuda: 'CUDA 版本',
    info_pytorch: 'Pytorch 版本',
    info_cpu: 'CPU 型號',
    info_gpu: 'GPU 型號',
    info_hardware: '計算硬體',
    client_online: '線上',
    client_offline: '離線',
    client_paused: '已暫停',
    client_queued: '排隊中',
    client_computing: '計算中',
    task_running: '執行中，進度 {p}%',
    cancel_task: '取消任務',
    task_cancelled: '任務已取消',
    task_cancel_failed: '取消失敗: {msg}',
    task_paused: '已加入佇列，目前被管理員暫停',
    task_failed_status: '任務失敗: {msg}',
    task_timeout_status: '任務逾時，請重試',
    task_expired_status: '任務結果已過期，請重新執行',
    task_not_found: '任務不存在',
};

// en 词典
window.I18N_LOCALES = window.I18N_LOCALES || {};
window.I18N_LOCALES['en'] = {
    app_title: 'Pattern Formation Visualization System',
    loading: 'Loading...',
    model_select: 'Model Selection',
    params_setting: 'Parameter Settings',
    reset_params: 'Reset Parameters',
    init_range: 'Initial Value Range',
    x_min: 'X Min',
    x_max: 'X Max',
    y_min: 'Y Min',
    y_max: 'Y Max',
    apply_best_init: 'Apply Recommended',
    track_points: 'Evolution Tracking Points',
    coord_x: 'X Coordinate',
    coord_y: 'Y Coordinate',
    add: 'Add',
    clear: 'Clear',
    edit: 'Edit',
    delete: 'Delete',
    iterations: 'Iterations',
    reset_iters: 'Apply Recommended',
    anim_setting: 'Animation Settings',
    anim_start: 'Start Iteration',
    anim_end: 'End Iteration',
    anim_frames: 'Frames',
    control_panel: 'Control Panel',
    run_sim: 'Run Simulation',
    run_anim: 'Animation Simulation',
    reset_all: 'Reset All',
    clean_cache: 'Clear Cache',
    software_settings: 'Software Settings',
    language: 'Language',
    hardware_label: 'Computing Hardware',
    client_label: 'Client',
    client_name: 'Client Name',
    language_zh_cn: 'Simplified Chinese',
    language_zh_tw: 'Traditional Chinese',
    language_en: 'English',
    language_ja: 'Japanese',
    language_ko: 'Korean',
    restore_default: 'Restore Default',
    save: 'Save',
    cancel: 'Cancel',
    status_ready: 'Ready, select parameters to run',
    tab_2d: '2D Pattern',
    tab_3d: '3D Pattern',
    tab_anim: 'Animation',
    play: 'Play',
    pause: 'Pause',
    speed: 'Speed',
    computing: 'Computing...',
    point_range_error: 'Coordinates must be between 0 and 99',
    iter_max_warning: 'Iterations cannot exceed {max}, automatically adjusted to {max}',
    point_exists: 'Point ({x},{y}) already exists',
    simulating_status: 'Simulation in progress...',
    sim_complete: 'Simulation complete — {model}, {iters} iterations',
    sim_done: 'Simulation complete!',
    sim_failed: 'Simulation failed: {msg}',
    init_failed: 'Initialization failed',
    restored_sim: 'Previous simulation result restored',
    restored_toast: 'Previous result restored',
    reset_done: 'All settings have been reset',
    status_reset_all: 'This client has reset all settings',
    status_cache_cleared: 'This client cache has been cleared',
    status_cache_clear_failed: 'This client cache cleanup failed',
    status_task_cancel_requested: 'Cancellation requested for this client task',
    status_settings_saved: 'This client software settings have been saved',
    client_task_log_title: 'This Client Task Logs',
    client_task_log_desc: 'Only task logs submitted by this browser client are shown.',
    client_task_log_close: 'Close',
    client_task_log_loading: 'Loading this client task logs...',
    client_task_log_empty: 'No task logs are available for this client.',
    clean_failed: 'Cleanup failed: {msg}',
    need_anim: 'Please run animation first',
    anim_ready: 'Animation data ready',
    anim_ready_status: 'Animation ready, click play',
    anim_failed_status: 'Animation preparation failed',
    anim_preparing: 'Preparing animation data...',
    track_list_center: 'Tracking: center (50,50)',
    frame_count: 'Frame: {current} / {total}',
    iterations_axis: 'Iterations',
    density_axis: 'Population Density',
    axis_x: 'X Axis',
    axis_y: 'Y Axis',
    center_evo_title: 'Center Point Evolution',
    center_x: 'X Population - Center',
    center_y: 'Y Population - Center',
    density: 'Density',
    anim_title_x: 'X Population - Iteration {iter}',
    anim_title_y: 'Y Population - Iteration {iter}',
    anim_title_combined: 'Combined Pattern - Iteration {iter}',
    model_1: 'Model 1·R-M',
    model_2: 'Model 2·Holling II',
    model_3: 'Model 3·Ratio-Dependent',
    model_4: 'Model 4·Symmetric Competition',
    model_5: 'Model 5·Continuous-Discrete',
    task_queued: 'Queued, {n} task(s) ahead',
    status_queued: 'Queued',
    status_running: 'Running',
    status_completed: 'Completed',
    status_cancelled: 'Cancelled',
    status_timeout: 'Timed out',
    status_failed: 'Failed',
    status_expired: 'Expired',
    type_simulate: 'Simulation',
    type_animate: 'Animation',
    info_version: 'Software Version',
    info_client_id: 'Client ID',
    info_python: 'Python Version',
    info_cuda: 'CUDA Version',
    info_pytorch: 'PyTorch Version',
    info_cpu: 'CPU Model',
    info_gpu: 'GPU Model',
    info_hardware: 'Compute Hardware',
    client_online: 'Online',
    client_offline: 'Offline',
    client_paused: 'Paused',
    client_queued: 'Queued',
    client_computing: 'Computing',
    task_running: 'Running, {p}% complete',
    cancel_task: 'Cancel Task',
    task_cancelled: 'Task cancelled',
    task_cancel_failed: 'Cancel failed: {msg}',
    task_paused: 'Queued, currently paused by administrator',
    task_failed_status: 'Task failed: {msg}',
    task_timeout_status: 'Task timed out, please retry',
    task_expired_status: 'Task result expired, please run again',
    task_not_found: 'Task not found',
};

// ja 词典
window.I18N_LOCALES = window.I18N_LOCALES || {};
window.I18N_LOCALES['ja'] = {
    app_title: 'パターン形成可視化システム',
    loading: '読み込み中...',
    model_select: 'モデル選択',
    params_setting: 'パラメータ設定',
    reset_params: 'パラメータをリセット',
    init_range: '初期値範囲',
    x_min: 'X 最小値',
    x_max: 'X 最大値',
    y_min: 'Y 最小値',
    y_max: 'Y 最大値',
    apply_best_init: '推奨値を適用',
    track_points: '時間発展トラッキングポイント',
    coord_x: 'X座標',
    coord_y: 'Y座標',
    add: '追加',
    clear: 'クリア',
    edit: '編集',
    delete: '削除',
    iterations: '反復回数',
    reset_iters: '推奨値を適用',
    anim_setting: 'アニメーション設定',
    anim_start: '開始反復',
    anim_end: '終了反復',
    anim_frames: 'フレーム数',
    control_panel: 'コントロールパネル',
    run_sim: 'シミュレーション実行',
    run_anim: 'アニメーション実行',
    reset_all: 'すべてリセット',
    clean_cache: 'キャッシュをクリア',
    software_settings: 'ソフトウェア設定',
    language: '言語',
    hardware_label: '計算ハードウェア',
    client_label: 'クライアント',
    client_name: 'クライアント名',
    language_zh_cn: '簡体字中国語',
    language_zh_tw: '繁体字中国語',
    language_en: '英語',
    language_ja: '日本語',
    language_ko: '韓国語',
    restore_default: 'デフォルトに戻す',
    save: '保存',
    cancel: 'キャンセル',
    status_ready: '準備完了、パラメータを選択して実行してください',
    tab_2d: '2次元パターン',
    tab_3d: '3次元パターン',
    tab_anim: 'アニメーション',
    play: '再生',
    pause: '一時停止',
    speed: '速度',
    computing: '計算中...',
    point_range_error: '座標は0~99の範囲でなければなりません',
    iter_max_warning: '反復回数は{max}を超えられないため、自動的に{max}に調整しました',
    point_exists: '点({x},{y})は既に存在します',
    simulating_status: 'シミュレーション実行中...',
    sim_complete: 'シミュレーション完了 — {model}、反復{iters}回',
    sim_done: 'シミュレーション完了！',
    sim_failed: 'シミュレーション失敗: {msg}',
    init_failed: '初期化失敗',
    restored_sim: '前回のシミュレーション結果を復元しました',
    restored_toast: '前回の結果を復元しました、リロード不要',
    reset_done: 'すべての設定がリセットされました',
    status_reset_all: 'このクライアントの設定をすべてリセットしました',
    status_cache_cleared: 'このクライアントのキャッシュを消去しました',
    status_cache_clear_failed: 'このクライアントのキャッシュ消去に失敗しました',
    status_task_cancel_requested: 'このクライアントのタスク取消を要求しました',
    status_settings_saved: 'このクライアントのソフトウェア設定を保存しました',
    client_task_log_title: 'このクライアントのタスクログ',
    client_task_log_desc: 'このウィンドウのクライアントが送信したタスクログのみを表示します。',
    client_task_log_close: '閉じる',
    client_task_log_loading: 'このクライアントのタスクログを読み込んでいます...',
    client_task_log_empty: 'このクライアントに表示できるタスクログはありません。',
    clean_failed: 'クリア失敗: {msg}',
    need_anim: '先にアニメーション計算を実行してください',
    anim_ready: 'アニメーションデータ準備完了',
    anim_ready_status: 'アニメーションデータ準備完了、再生をクリック',
    anim_failed_status: 'アニメーション準備失敗',
    anim_preparing: 'アニメーションデータを準備中...',
    track_list_center: '現在のトラッキングポイント: 中心点(50,50)',
    frame_count: 'フレーム: {current} / {total}',
    iterations_axis: '反復回数',
    density_axis: '個体群密度',
    axis_x: 'X軸',
    axis_y: 'Y軸',
    center_evo_title: '中心点の時間発展',
    center_x: 'X個体群-中心点',
    center_y: 'Y個体群-中心点',
    density: '密度',
    anim_title_x: 'X個体群 - 反復 {iter}',
    anim_title_y: 'Y個体群 - 反復 {iter}',
    anim_title_combined: '結合パターン - 反復 {iter}',
    model_1: 'モデル1·R-M型',
    model_2: 'モデル2·Holling II型',
    model_3: 'モデル3·比依存型',
    model_4: 'モデル4·対称競争',
    model_5: 'モデル5·連続化離散型',
    task_queued: 'キューに追加されました。残り {n} 件',
    status_queued: '待機中',
    status_running: '実行中',
    status_completed: '完了',
    status_cancelled: 'キャンセル済み',
    status_timeout: 'タイムアウト',
    status_failed: '失敗',
    status_expired: '期限切れ',
    type_simulate: 'シミュレーション',
    type_animate: 'アニメーション',
    info_version: 'ソフトウェアバージョン',
    info_client_id: 'クライアント ID',
    info_python: 'Python バージョン',
    info_cuda: 'CUDA バージョン',
    info_pytorch: 'PyTorch バージョン',
    info_cpu: 'CPU 型番',
    info_gpu: 'GPU 型番',
    info_hardware: '計算ハードウェア',
    client_online: 'オンライン',
    client_offline: 'オフライン',
    client_paused: '一時停止',
    client_queued: '待機中',
    client_computing: '計算中',
    task_running: '実行中、進捗 {p}%',
    cancel_task: 'タスクをキャンセル',
    task_cancelled: 'タスクをキャンセルしました',
    task_cancel_failed: 'キャンセル失敗: {msg}',
    task_paused: 'キューに追加されました。現在管理者により一時停止中です',
    task_failed_status: 'タスク失敗: {msg}',
    task_timeout_status: 'タスクがタイムアウトしました。再試行してください',
    task_expired_status: 'タスク結果の有効期限が切れました。再実行してください',
    task_not_found: 'タスクが見つかりません',
};

// ko 词典
window.I18N_LOCALES = window.I18N_LOCALES || {};
window.I18N_LOCALES['ko'] = {
    app_title: '패턴 형성 시각화 시스템',
    loading: '로딩 중...',
    model_select: '모델 선택',
    params_setting: '매개변수 설정',
    reset_params: '매개변수 초기화',
    init_range: '초기값 범위',
    x_min: 'X 최소값',
    x_max: 'X 최대값',
    y_min: 'Y 최소값',
    y_max: 'Y 최대값',
    apply_best_init: '권장값 적용',
    track_points: '시간 진화 추적 지점',
    coord_x: 'X좌표',
    coord_y: 'Y좌표',
    add: '추가',
    clear: '비우기',
    edit: '편집',
    delete: '삭제',
    iterations: '반복 횟수',
    reset_iters: '권장값 적용',
    anim_setting: '애니메이션 설정',
    anim_start: '시작 반복',
    anim_end: '종료 반복',
    anim_frames: '프레임 수',
    control_panel: '제어판',
    run_sim: '시뮬레이션 실행',
    run_anim: '애니메이션 실행',
    reset_all: '전체 초기화',
    clean_cache: '캐시 비우기',
    software_settings: '소프트웨어 설정',
    language: '언어',
    hardware_label: '계산 하드웨어',
    client_label: '클라이언트',
    client_name: '클라이언트 이름',
    language_zh_cn: '간체 중국어',
    language_zh_tw: '번체 중국어',
    language_en: '영어',
    language_ja: '일본어',
    language_ko: '한국어',
    restore_default: '기본값 복원',
    save: '저장',
    cancel: '취소',
    status_ready: '준비 완료, 매개변수를 선택하여 실행하세요',
    tab_2d: '2D 패턴',
    tab_3d: '3D 패턴',
    tab_anim: '애니메이션',
    play: '재생',
    pause: '일시정지',
    speed: '속도',
    computing: '계산 중...',
    point_range_error: '좌표는 0~99 사이여야 합니다',
    iter_max_warning: '반복 횟수는 {max}를 초과할 수 없어 자동으로 {max}로 조정되었습니다',
    point_exists: '점({x},{y})이 이미 존재합니다',
    simulating_status: '시뮬레이션 실행 중...',
    sim_complete: '시뮬레이션 완료 — {model}, 반복 {iters}회',
    sim_done: '시뮬레이션 완료!',
    sim_failed: '시뮬레이션 실패: {msg}',
    init_failed: '초기화 실패',
    restored_sim: '이전 시뮬레이션 결과가 복원되었습니다',
    restored_toast: '이전 결과가 복원되었습니다, 새로고침 불필요',
    reset_done: '모든 설정이 초기화되었습니다',
    status_reset_all: '이 클라이언트의 모든 설정을 초기화했습니다',
    status_cache_cleared: '이 클라이언트의 캐시를 정리했습니다',
    status_cache_clear_failed: '이 클라이언트의 캐시 정리에 실패했습니다',
    status_task_cancel_requested: '이 클라이언트 작업의 취소를 요청했습니다',
    status_settings_saved: '이 클라이언트의 소프트웨어 설정을 저장했습니다',
    client_task_log_title: '이 클라이언트 작업 로그',
    client_task_log_desc: '이 창의 클라이언트가 제출한 작업 로그만 표시합니다.',
    client_task_log_close: '닫기',
    client_task_log_loading: '이 클라이언트의 작업 로그를 불러오는 중...',
    client_task_log_empty: '이 클라이언트에 표시할 작업 로그가 없습니다.',
    clean_failed: '비우기 실패: {msg}',
    need_anim: '먼저 애니메이션 계산을 실행하세요',
    anim_ready: '애니메이션 데이터 준비 완료',
    anim_ready_status: '애니메이션 데이터 준비 완료, 재생을 클릭하세요',
    anim_failed_status: '애니메이션 준비 실패',
    anim_preparing: '애니메이션 데이터를 준비하는 중...',
    track_list_center: '현재 추적 지점: 중심점(50,50)',
    frame_count: '프레임: {current} / {total}',
    iterations_axis: '반복 횟수',
    density_axis: '개체군 밀도',
    axis_x: 'X축',
    axis_y: 'Y축',
    center_evo_title: '중심점 시간 진화',
    center_x: 'X 개체군-중심점',
    center_y: 'Y 개체군-중심점',
    density: '밀도',
    anim_title_x: 'X 개체군 - 반복 {iter}',
    anim_title_y: 'Y 개체군 - 반복 {iter}',
    anim_title_combined: '결합 패턴 - 반복 {iter}',
    model_1: '모델1·R-M형',
    model_2: '모델2·Holling II형',
    model_3: '모델3·비율 의존형',
    model_4: '모델4·대칭 경쟁',
    model_5: '모델5·연속화 이산형',
    task_queued: '대기열에 추가되었습니다. 앞에 {n}개의 작업이 있습니다',
    status_queued: '대기 중',
    status_running: '실행 중',
    status_completed: '완료됨',
    status_cancelled: '취소됨',
    status_timeout: '시간 초과',
    status_failed: '실패함',
    status_expired: '만료됨',
    type_simulate: '시뮬레이션',
    type_animate: '애니메이션',
    info_version: '소프트웨어 버전',
    info_client_id: '클라이언트 ID',
    info_python: 'Python 버전',
    info_cuda: 'CUDA 버전',
    info_pytorch: 'PyTorch 버전',
    info_cpu: 'CPU 모델',
    info_gpu: 'GPU 모델',
    info_hardware: '연산 하드웨어',
    client_online: '온라인',
    client_offline: '오프라인',
    client_paused: '일시중지',
    client_queued: '대기 중',
    client_computing: '연산 중',
    task_running: '실행 중, 진행률 {p}%',
    cancel_task: '작업 취소',
    task_cancelled: '작업이 취소되었습니다',
    task_cancel_failed: '취소 실패: {msg}',
    task_paused: '대기열에 추가되었습니다. 현재 관리자가 일시 중지했습니다',
    task_failed_status: '작업 실패: {msg}',
    task_timeout_status: '작업 시간 초과, 다시 시도하세요',
    task_expired_status: '작업 결과가 만료되었습니다. 다시 실행하세요',
    task_not_found: '작업을 찾을 수 없습니다',
};

/**
 * i18n 翻译模块 - 根据操作系统语言自动适配界面文字
 * 支持：简体中文(默认)、繁体中文、英文、日语、韩语（其他语言一律显示英文）
 * 词典与翻译逻辑合并在本文件，前后台共用。
 */
const i18n = (() => {
    const translations = window.I18N_LOCALES || {};
    // 支持的语言列表
    const supportedLangs = ['zh-CN', 'zh-TW', 'en', 'ja', 'ko'];

    /**
     * 检测系统语言
     * @returns {string} 语言代码
     */
    function detectLang() {
        const nav = (navigator.language || 'zh-CN').toLowerCase();
        if (nav.startsWith('zh')) {
            // 繁体中文系统（台湾/香港/澳门）用繁体
            if (nav.startsWith('zh-tw') || nav.startsWith('zh-hk') || nav.startsWith('zh-mo')) return 'zh-TW';
            return 'zh-CN';
        }
        if (nav.startsWith('ja')) return 'ja';
        if (nav.startsWith('ko')) return 'ko';
        return 'en';  // 其他语言系统一律显示英文
    }

    // 当前语言（zh-CN / zh-TW / en / ja / ko）
    // 优先使用保存的语言，否则使用检测到的语言
    let lang = localStorage.getItem('app_language') || detectLang();

    /**
     * 翻译函数，支持 {变量} 占位符
     * @param {string} key - 字典键
     * @param {Object} params - 替换参数
     * @returns {string} 翻译后的文本
     */
    function t(key, params) {
        let text = translations[lang][key] || translations['zh-CN'][key] || key;
        if (params) {
            for (const k in params) {
                text = text.replaceAll(`{${k}}`, params[k]);
            }
        }
        return text;
    }

    /**
     * 应用到页面：替换data-i18n元素的文本，设置html lang和标题
     */
    function applyTranslations() {
        document.documentElement.lang = lang;
        document.querySelectorAll('[data-i18n]').forEach(el => {
            el.textContent = t(el.dataset.i18n);
        });
        document.querySelectorAll('[data-i18n-placeholder]').forEach(el => {
            el.placeholder = t(el.dataset.i18nPlaceholder);
        });
        document.title = t('app_title');
        // 同步自定义下拉组件选中项文本
        document.querySelectorAll('.custom-select').forEach(cs => {
            const selected = cs.querySelector('.custom-select-option.selected');
            const text = cs.querySelector('.selected-text');
            if (selected && text) text.textContent = selected.textContent;
        });
    }

    /**
     * 设置语言
     * @param {string} newLang - 新语言代码
     */
    function setLang(newLang) {
        if (supportedLangs.includes(newLang)) {
            lang = newLang;
            localStorage.setItem('app_language', newLang);
            applyTranslations();
            window.dispatchEvent(new CustomEvent('app-language-changed', { detail: { lang: newLang } }));
        }
    }

    /**
     * 获取当前语言
     * @returns {string} 当前语言代码
     */
    function getLang() {
        return lang;
    }

    return { t, applyTranslations, lang, setLang, getLang };
})();

// DOM加载完成后立即应用翻译（在app.js的init之前执行）
document.addEventListener('DOMContentLoaded', () => i18n.applyTranslations());
